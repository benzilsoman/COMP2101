...
hostname=$(hostname)
a=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
b=$(ip a s $a |awk '/inet /{gsub(/\/.*/,"");print $2}')
LAddress=$b
c=$(hostname -I)
d=$(getent hosts $c | awk '{print $2}')
LHostname=$d
ExIp=$(curl -s icanhazip.com)
g=$(getent hosts $ExIp | awk '{print $2}')
ExName=$g
Raddress=$(ip r s default| awk '{print $3}')
Rhostname=$(route |grep "default" |awk '{ print $2}')
cat <<EOF
Hostname        : $hostname
